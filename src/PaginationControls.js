...
import {PaginationButtons} from "../src/shop/PaginationButtons";

export class PaginationControls extends Component {
  constructor(props) {
    super(props);
    this.pageSizes = this.props.sizes || [5, 10, 26, 100];
    this.sortKeys = this.props.keys || ["Name", "Price"];
  }

  handlePageSizeChange = (ev) => {
    this.props.setPageSize(ev.target.value); //setPageSize iz action creatora
  };

  // onChange prop is applied to the select elements to respond to user changes. which are handles by methods
  //that receive the event triggered and INVOKE function props that are received from the parent component.
